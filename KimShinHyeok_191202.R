...
#1-2
# score의 열 이름을 각각 male, female로 바꾸시오
...
#2-1
# state.x77을 변환하여 st에 data frame으로 작성
data()
...
#2-11
# Texas 주의 면적(Area)을 출력
st['Texas', 'Area']

##st <- subset(st['Texas',],select = "Area"); st
...
#2-18
# 문맹률(Illiteracy)이 2.0 이상인 주의 평균 수입은 얼마인지 출력
...
# 5-1
# R에서 제공하는 state.x77 데이터셋에서 수입(Income)이 5,000 이상인
...
# 5-2
# 1.에서 만든 rich_state.csv파일을 읽어서 ds 변수에 저장한 후 ds 내용 출력
setwd('C:/Rstudy/work123')
